<!-- <form method="POST" autocomplete="off" action="<?= base_url('wawancara/simpan_jenjang') ?>"> -->
...
<div class="modal-body">
    ...
    <div class="row">
        ...
        <div class="col-12 mb-12 mb-2">
            <a type="button" href="<?= base_url('form_wawancara?id=' . encode_id($jenjangs['id'])) ?>" class="btn btn-outline-primary">Form Pengisian Hasil Wawancara Jenjang <?= strtoupper($jenjangs['jenjang']) ?></a>
        </div>
        <?php } ?>
    </div>
</div>
...
function save_jenjang(jenis, id) {
    ...
    if (message) {
        window.location.href = ("<?= base_url('wawancara/simpan_jenjang'); ?>") + "/" + id + "/" + jenis + "/" + jenjang
    } else return false
