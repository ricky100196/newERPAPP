...
let myDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
    url: "<?= base_url('supervisor/upload_multi/' . encode_id($instrumen->id).'/'. encode_id('4')) ?>", // Set the url
    thumbnailWidth: 80,
    thumbnailHeight: 80,
    parallelUploads: 20,
...
function delete_upload(id) {
    $.ajax({
        type: 'POST',
        url: "<?= base_url('supervisor/delete_upload'); ?>",
        data: {
            id
        },
        cache: false,
...
function simpan_submit() {
	        Swal.fire({
	            ...
	            if (result.value) {
	                window.location.href = '<?= base_url('supervisor/simpan_submit') ?>';
	            }
	        });
	    }
</script>