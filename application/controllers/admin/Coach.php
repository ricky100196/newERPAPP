...
class Coach extends MY_Controller
{
	...
	{
		parent::__construct();
		// $this->load->model('admin/coach/M_coach_model' => 'm_main');
		$this->load->model('admin/coach/M_coach_model', 'm_main');
	}
	...
	public function index()
	{
		...
		$data = [
			'scriptExtra' => 'admin/coach/js',
			'btn_tambah'  => '<a href="' . site_url('admin/coach/add') . '" class="btn btn-sm btn-rounded btn-outline-primary"><i class="icon-plus"></i> Tambah</a>'
		];
		$view['title'] = 'Data Coach';
		$view['content'] = 'admin/coach/index';
	...
	function get_data_table()
	{
		...
		foreach ($list as $field) {
			...
			$row[] =   '<a href="' . site_url('admin/coach/edit/' . $field->id) . '" class="btn btn-sm btn-primary">Edit</a>&nbsp;
			<a href="javascript:;" data-url="'.site_url('admin/coach/hapus/' . $field->id).'" class="btn btn-sm btn-rounded btn-danger mr-1 mb-0" onclick="doHapus(this)"> NONAKTIF </a>';
	...
	{
		$id = decode_id($id);
		$cek_exist = $this->db->get_where('coach_data', ['id' => $id]);
		if ($aksi == 'hapus') {
			$dt = array(
		...
		if ($cek_exist->num_rows() > 0) {
			$this->db->where('id', $id)->update('coach_data', $dt);
		}
	...
	function add()
	{
		...
		$data['scriptExtra'] 	= 'admin/coach/form_js';
		$data['id']         	= '';
		$data['provinsi']   	= $provinsi;
		$view['title'] 			= 'Data Coach';
		$view['content'] 		= 'admin/coach/form';
		$this->template->display($view, $data);
	}
	...
	function simpan()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->db->insert("coach_data", $data_insert) == TRUE) {
				$response = [
					'status' => true,
	...
	function update()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->db->update("coach_data", $data_insert) == TRUE) {
				$response = [
					'status' => true,
					'pesan' => 'Data Tersimpan'
				];
	...
	{
		$this->db->where('id', $id)->update('coach_data', [
			'deleted' => date('Y-m-d H:i:s'),
		]);
	...
	public function cek_lulusan() {
		...
		if ($user->num_rows() > 0) {
			...
			if ($api_lulusan != null) {
				...
				$kode_kab = $this->db->from('ref_kabupaten')->where('nama', $api_lulusan->kab)->get()->row();
				$api_lulusan->kode_kab = @$kode_kab->kode_wilayah;
				$kode_prov = $this->db->from('ref_provinsi')->like('nama', $api_lulusan->prov,'both')->get()->row();
				$api_lulusan->kode_prov = $kode_prov->kode_wilayah;
